...
! function(e, t) {
    ...
    function E(v, E) {
        ...
        function R() {
            ...
            // if play-large and fullscreen is enabled then return the custom control setup
            //  but if there are other controls enabled then default to the original control setup
            if (s(E.controls, "play-large") && s(E.controls, "fullscreen") && !s(E.controls, "play")  && !s(E.controls, "progress") && !s(E.controls, "current-time") && !s(E.controls, "mute") && !s(E.controls, "volume") && !s(E.controls, "captions")) {
                // move fullscreen to the bottom right corner
                return s(E.controls, "play-large") && e.push('<button type="button" data-plyr="play" class="plyr__play-large" title="Click to play">', '<svg><use xlink:href="' + n + '-play" /></svg>', '<span class="plyr__sr-only">' + E.i18n.play + "</span>", "</button>"), e.push('<div class="plyr__controls single">'), s(E.controls, "fullscreen") && e.push('<button type="button" data-plyr="fullscreen">', '<svg class="icon--exit-fullscreen"><use xlink:href="' + n + '-exit-fullscreen" /></svg>', '<svg><use xlink:href="' + n + '-enter-fullscreen" /></svg>', '<span class="plyr__sr-only">' + E.i18n.toggleFullscreen + "</span>", "</button>"), e.push("</div>"), e.join("")
            } else { 
